				...
				.setTooltipSupplier(() -> {
					if (Backends.supportsGpuAcceleration()) {
						...
						return Optional.of(new MutableComponent[]{
							...
							Component.translatable("config.asyncparticles.incompatibility.backend", RenderSystem.getDevice().getBackendName())
	...
	private static void buildMixinCategory(ConfigBuilder builder, ConfigEntryBuilder entryBuilder, ConfigEntryBuilder revertButtonEntryBuilder, MixinConfigBundle mixinBundle) {
		...
		mixinCategory.addEntry(modifyOriginal(new StringListListEntryFixRestart(revertButtonEntryBuilder
			.startStrList(Component.translatable("config.asyncparticles.mixin.particle.asyncTickableGroup"),
				List.copyOf(displayMixinConfig.getAsyncTickableParticleGroups()))
			.setDefaultValue(List.copyOf(originalMixinConfig.getAsyncTickableParticleGroups()))
			.setSaveConsumer(l -> {
				LinkedHashSet<String> s = new LinkedHashSet<>(l);
				s.addAll(defaultMixinConfig.getAsyncTickableParticleGroups());
				displayMixinConfig.setAsyncTickableParticleGroups(Collections.unmodifiableSet(s));
			})
			.setTooltip(
				Component.translatable("text.cloth-config.restart_required")
					.withStyle(ChatFormatting.DARK_RED),
				Component.translatable("config.asyncparticles.tooltip.list-how-to-reset"))
			.requireRestart()
			.build()), originalMixinConfig.getAsyncTickableParticleGroups()));
		mixinCategory.addEntry(modifyOriginal(new StringListListEntryFixRestart(revertButtonEntryBuilder
