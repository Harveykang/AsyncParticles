...
import static fun.qu_an.minecraft.asyncparticles.client.core.particle.gpu_acceleration.GpuParticlePipelines.IDENTITY_PARTICLE;
import static fun.qu_an.minecraft.asyncparticles.client.core.particle.gpu_acceleration.GpuParticlePipelines.RAW_PARTICLE;
...
public class VkCompParticleRenderer implements IParticleRenderer {
	...
	private void extractAppendParticles(Vec3 prevGpuCamPos, List<SingleQuadParticle> pending, List<LayerBatch> batches) {
		...
		for (int i = 0, pendingSize = Math.min(particleLimit, appendCount); i < pendingSize; i++) {
			...
			layerMap.computeIfAbsent(((GpuParticleAddon) p).asyncparticles$getLayer(), a -> new ReferenceArrayList<>(appendCount / 2)).add(p);
		}
...
//			waitReady();
		...
		private void createTargetBuffer(int size) {
			...
			Buffer buffer = new Buffer("ParticleTargetBuffer", GpuBuffer.USAGE_VERTEX, MemoryTypes.GPU_MEM);
