 ...
 * Thanks to ChatGPT
 */
public class ContraptionRainBlockingOptimized {
	/**
	 * ------------------------------------------------------------------------
	 ...
	 * <p>
	 * 结果语义：
	 * heightMap[(x, z)] = 当前该世界列的最高挡雨面 worldY
	 * <p>
	 * ------------------------------------------------------------------------
	 * 最初未优化版（保留对照，不参与运行）
	 * ------------------------------------------------------------------------
	 ...
	 * rasterizeTopQuadFace(heightMap, minX, maxX, minZ, maxZ,
	 ...
	 * ------------------------------------------------------------------------
	 * <p>
	 * // 预计算 local -> world 仿射变换：
	 * // world = T * local + offset
	 * <p>
	 * double[] cornerX = new double[8];
	 * double[] cornerY = new double[8];
	 ...
	 * 对局部 AABB：
	 * - 先把“局部中心”变到世界
	 * - 再把局部三个半轴 extents 变成世界空间中的 3 个向量 vx/vy/vz
	 * - 8 个角点直接由：
	 * centerWorld ± vx ± vy ± vz
	 * 组合得到
	 * <p>
	 * 这样可把“8 次完整仿射变换”压成：
	 * - 1 次中心变换
	...
	public static void batchCheckRainBlocking(Long2FloatMap heightMap,
		...
		for (Iterator<AbstractContraptionEntity> it = forEachContraption(level); it.hasNext(); ) {
			...
			AccessorMatrix3d mAcc = (AccessorMatrix3d) matrix;
			Vec3 anchor = entity.getAnchorVec();
			float yawOffset = rotationState.getYawOffset();

			/*
			 * --------------------------------------------------------------------
			 * 最初的 localToWorldPos（保留作公式对照，不参与运行）
			 * --------------------------------------------------------------------
			 *
			 ...
			 * 上式可整理成：
			 *
			 *   world = Ry(+yaw) * M^T * local
			 *         + [anchor + center - Ry(+yaw) * M^T * center]
			 *
			 * 所以可写成统一的仿射形式：
			 *
			 *   world = T * local + offset
			...
			// 注意：这里默认 AccessorMatrix3d 的元素访问器名是 getM00() ~ getM22()
			// 如果你项目里的 accessor 名字不同，请替换成实际名字。
			...
			for (int i = 0; i < colliders.size; i++) {
				...
				final double worldCenterX = t00 * cx + t01 * cy + t02 * cz + offsetX;
				final double worldCenterY = t10 * cx + t11 * cy + t12 * cz + offsetY;
				final double worldCenterZ = t20 * cx + t21 * cy + t22 * cz + offsetZ;

				// 局部 X 半轴 extent 变到世界后的向量 vx = T * (ex, 0, 0)
				final double vxX = t00 * ex;
				final double vxY = t10 * ex;
				final double vxZ = t20 * ex;

				// 局部 Y 半轴 extent 变到世界后的向量 vy = T * (0, ey, 0)
				final double vyX = t01 * ey;
				final double vyY = t11 * ey;
				final double vyZ = t21 * ey;

				// 局部 Z 半轴 extent 变到世界后的向量 vz = T * (0, 0, ez)
				final double vzX = t02 * ez;
				final double vzY = t12 * ez;
				final double vzZ = t22 * ez;
				...
				for (int f = 0; f < BOX_FACE_INDICES_FLAT.length; f += 4) {
					rasterizeTopQuadFaceRawOptimized(
						tempHeightMap,
						queryMinX, queryMaxX,
						queryMinZ, queryMaxZ,
						cornerX, cornerY, cornerZ,
				...
				if (!doRainEffectsIfMoving && !tempHeightMap.isEmpty()) {
					for (Long2FloatMap.Entry e : tempHeightMap.long2FloatEntrySet()) {
						...
						heightMap.put(l, f);
						boolean b = entity.getContactPointMotion(new Vec3(BlockPos.getX(l), f, BlockPos.getZ(l))).lengthSqr() > 0.01;
						isMoving.put(l, b);
					}
					tempHeightMap.clear();
				}
	 ...
	 */
	private static void rasterizeTopQuadFaceRawOptimized(
...
//		final double minFaceY = Math.min(Math.min(ay, by), Math.min(cy, dy));
		...
		final int fromX = Math.max(queryMinX, Mth.floor(minFaceX));
		final int toX = Math.min(queryMaxX, Mth.ceil(maxFaceX) - 1);
		final int fromZ = Math.max(queryMinZ, Mth.floor(minFaceZ));
		final int toZ = Math.min(queryMaxZ, Mth.ceil(maxFaceZ) - 1);
