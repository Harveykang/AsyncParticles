package fun.qu_an.minecraft.asyncparticles.client.compat.particlerain.v3.neoforge;
...
public class ParticleRainCompatImpl extends ParticleRainCompat {
	...
	public void onShipCollision(ClientLevel level, Vec3 location, Vec3 movement, AABB aabb) {
		...
		if (ParticleRainConfig.doRippleParticles && fluidState.isSourceOfType(Fluids.WATER)) {
			...
			if (particle != null) {
				...
				((RippleParticleAddon) particle).asycncparticles$setNormal(normal1);
			}
	...
	public void onCreateCollision(@NotNull ClientLevel level, Vec3 originalMotion, @NotNull Vec3 clipMotion, @NotNull AABB aabb) {
		...
		if (createRainEffect != RainEffect.NONE && ParticleRainConfig.doSplashParticles) {
