...
public class PrettyRainCompat {
	public static AtomicInteger particleCount = new AtomicInteger();
	public static AtomicInteger fogCount = new AtomicInteger();
	...
	public static void onCreateCollision(@NotNull ClientLevel level, Vec3 originalMotion, @NotNull Vec3 clipMotion, @NotNull AABB aabb) {
		...
		if (createRainEffect != RainEffect.NONE && config.doSplashParticles) {
			...
			if (CollideUtil.isCollideWithContraptions(level, motion1, aabb1, false).canSpawnRainEffect(createRainEffect)) {
